...
public enum ServiceExceptionMessages {
  ...
  SERVER_UNKNOWN_ERROR_EXTERNAL_API(HttpStatus.INTERNAL_SERVER_ERROR, "SERVER_UNKNOWN_ERROR_EXTERNAL_API"),
  // NOTE : 404
  ...
  // NOTE : 400
  BAD_REQUEST_QUERY_PARAMETER_REQUIRED(HttpStatus.BAD_REQUEST, "BAD_REQUEST_QUERY_PARAMETER_REQUIRED"),
  // NOTE : 2XX
  NO_CONTENT(HttpStatus.NO_CONTENT, "This response does not display the body. So, if possible, 200 (OK) should be used instead."),;

  private static final Map<String, ServiceExceptionMessages> descriptions =
      Collections.unmodifiableMap(Stream.of(values())
          .collect(Collectors.toMap(ServiceExceptionMessages::getDescription, Function.identity())));

...
public class LogicTestCase {
  ...
  public void placeSearch1Test() {
    ...
    List<String> placeNames =
        list.stream().map(Documents::getPlaceName).collect(Collectors.toList());

    Mono<String> actualMono = Mono.just("[카카오뱅크, 국민은행, 부산은행, 우리은행, 새마을금고, 하나은행, 기업은행]");
    StepVerifier.create(actualMono)
        .expectSubscription()
        .expectNext(placeNames.toString())
        .verifyComplete();
  }
  ...
  public void placeSearch3Test() {
    ...
    List<String> placeNames =
        places.stream().map(Places::getPlaceName).collect(Collectors.toList());
  ...
  public void removeWhiteSpace() {
    ...
    StepVerifier.create(actualMono)
        .expectSubscription()
        .expectNext(StringUtil.removeWhiteSpace(text, text2))
        .verifyComplete();
  }
  ...
  public void removeHtmlTag() {
    ...
    StepVerifier.create(actualMono)
        .expectSubscription()
        .expectNext(StringUtil.removeHtmlTag(text))
        .verifyComplete();
  }

  /** 장소 와 지번 주소를 사용 하지만, 비슷한 문자열 이고 길이가 짧을 수록 정확도가 떨어진다. 같은 조건에서 정확도가 제일 높은걸 찾아 본다. */
  @Test
  public void normalizedLevenshteinTest() {
    ...
    double five =
        NumberUtil.decimalFormat(
            algorithms.similarity("해성막창집본점부산해운대구중동1732", "해성막창집본점부산광역시해운대구중동1732"), 2);
    log.info("{}", one);
    log.info("{}", two);
    log.info("{}", three);
    log.info("{}", four);
    log.info("{}", five);
    /** 1.0 0.91 0.56 0.27 0.86 */
  ...
  /**
   * jaroWinklerTest , jaroWinklerTest2 의 two 비교시 실제로 장소명이나 주소가 자세해질 수록 정확도 차이가 분명 해진다. 0.95가 현재로선
   * 적정값 일듯 하다.
   */
  @Test
  public void jaroWinklerTest() {
    /** 1.0 0.97 0.85 0.52 0.97 */
  ...
  public void jaroWinkler2Test() {
    /** 1.0 0.91 0.85 0.52 0.97 */
    JaroWinkler algorithms = new JaroWinkler();
    double one = NumberUtil.decimalFormat(algorithms.similarity("카카오뱅크본점서울강북", "카카오뱅크본점서울강북"), 2);
    double two =
        NumberUtil.decimalFormat(algorithms.similarity("카카오뱅크본점서울강북1745", "카카오뱅크분점서울강북1778"), 2);
    double three = NumberUtil.decimalFormat(algorithms.similarity("카카오뱅크서울강북", "카카오뱅크경기성남"), 2);
    double four = NumberUtil.decimalFormat(algorithms.similarity("카카오뱅크본점서울강북", "카카오스타일경기성남"), 2);
    double five =
        NumberUtil.decimalFormat(
  ...
  public void ngramTest() {
    ...
    double five =
        NumberUtil.decimalFormat(
            algorithms.distance("해성막창집본점부산해운대구중동1732", "해성막창집본점부산광역시해운대구중동1732"), 2);
    log.info("{}", one);
    log.info("{}", two);
    log.info("{}", three);
    log.info("{}", four);
  ...
  public void qgramTest() {
    ...
    log.info("{}", four);
    log.info("{}", five);
    log.info("{}", Double.compare(two, 0.5));
    Mono<Boolean> actualMono = Mono.just(Double.compare(one, 0.5) == 1);
    StepVerifier.create(actualMono).expectSubscription().expectNext(false).verifyComplete();
    actualMono = Mono.just(Double.compare(two, 0.5) == 1);
    StepVerifier.create(actualMono).expectSubscription().expectNext(true).verifyComplete();
    actualMono = Mono.just(Double.compare(three, 0.5) == 1);
    StepVerifier.create(actualMono).expectSubscription().expectNext(true).verifyComplete();
    actualMono = Mono.just(Double.compare(four, 0.5) == 1);
    StepVerifier.create(actualMono).expectSubscription().expectNext(true).verifyComplete();
    actualMono = Mono.just(Double.compare(five, 0.5) == 1);
    StepVerifier.create(actualMono).expectSubscription().expectNext(true).verifyComplete();
  }
}
